import Payment from "payment"

export function formatCreditCardNumber(value) 
{
    if (!value) return value

    const issuer = Payment.fns.cardType(value)
    const clearValue = clearNumber(value)
    let nextValue

    switch (issuer) 
    {
        case 'amex':
            nextValue = `${clearValue.slice(0, 4)} ${clearValue.slice(4, 10)} ${clearValue.slice(10, 15)}`
        break
        case 'dinersclub':
            nextValue = `${clearValue.slice(0, 4)} ${clearValue.slice(4, 10)} ${clearValue.slice(10, 14)}`
        break
        default:
            nextValue = `${clearValue.slice(0, 4)} ${clearValue.slice(4, 8)} ${clearValue.slice(8, 12)} ${clearValue.slice(12, 16)}`
        break
    }

    return nextValue.trim()
}

export function validateCardNumber(value)
{
    return Payment.fns.validateCardNumber(value)
}

export function validateCardExpiry(value)
{
    return Payment.fns.validateCardExpiry(value)
}

export function validateCardCVC(value, cardNumber)
{
    const issuer = Payment.fns.cardType(cardNumber)

    return Payment.fns.validateCardCVC(value, issuer)
}

export function formatExpirationDate(value) 
{
    let clearValue = clearNumber(value)
    let numberValue = parseInt(clearValue)
    let isSingleDigit = false

    if (clearValue.length === 1) 
    {
        if (numberValue > 1 && numberValue < 10) 
        {
            isSingleDigit = true
            clearValue = "0" + clearValue
        }
    }

    if (clearValue.length >= 3 || isSingleDigit) 
    {
        return `${clearValue.slice(0, 2)}/${clearValue.slice(2, 4)}`
    }

    return clearValue
}

export function formatCVC(value, prevValue, allValues = {}) 
{
    const clearValue = clearNumber(value)
    let maxLength = 4

    if (allValues.number) 
    {
        const issuer = Payment.fns.cardType(allValues.number)
        maxLength = issuer === 'amex' ? 4 : 3
    }

    return clearValue.slice(0, maxLength)
}

export function getCardIssuer(number)
{
    return Payment.fns.cardType(number) || 'unknown';
}

// internal functions 
function clearNumber(value = '') 
{
    return value.replace(/\D+/g, '')
}